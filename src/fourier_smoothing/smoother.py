...
def fourier_identity_smoother(
    ...
    *,
    ...
    smoothed[-1], normalizers[-1] = normalize_fourier_density(
        multiply_fourier_via_grid(f[-1], beta[-1], clip_negative=clip_negative_products)
    )

    for t in range(steps - 2, -1, -1):
        future = multiply_fourier_via_grid(ell[t + 1], beta[t + 1], clip_negative=clip_negative_products)
        ...
        smoothed[t], normalizers[t] = normalize_fourier_density(
            multiply_fourier_via_grid(f[t], beta_t, clip_negative=clip_negative_products)
        )
